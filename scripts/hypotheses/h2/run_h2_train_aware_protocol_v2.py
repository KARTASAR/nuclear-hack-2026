"""End-to-end orchestrator for H2 train-aware validation protocol.

Runs:
1) train-aware H2 battery builder,
2) leakage audit,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run full train-aware H2 validation protocol.")
    ...
    parser.add_argument("--lock-mk3-fallback-strategy", default="")
    ...
    battery_fp = out_root / "h2_train_aware_battery.csv"
    summary_fp = out_root / "h2_train_aware_summary.csv"
    run_manifest_fp = out_root / "h2_train_aware_run_manifest.csv"
    leak_fp = out_root / "h2_train_aware_leakage_report.csv"
    cfg_fp = out_root / "h2_train_aware_config.json"

    build_cmd = [
        py,
        "scripts/hypotheses/h2/run_h2_hierarchical_battery_train_aware_v2.py",
        "--sample-meta",
        ...
        "--h2-topk-1500",
        str(int(args.h2_topk_1500)),
        "--h2-topk-2900",
        str(int(args.h2_topk_2900)),
        "--h2-logreg-c",
        str(float(args.h2_logreg_c)),
        "--h2-logreg-max-iter",
        str(int(args.h2_logreg_max_iter)),
        "--h2-min-rows-per-region",
        str(int(args.h2_min_rows_per_region)),
        "--h2-min-classes-per-region",
        str(int(args.h2_min_classes_per_region)),
        "--h2-name",
        str(args.h2_name),
        "--fallback-name",
    ...
    if bool(args.include_h2_global_control):
        build_cmd.append("--include-h2-global-control")
    _run(build_cmd)
    ...
    _run(
        [
            py,
            "scripts/hypotheses/h2/check_h2_holdout_leakage_v2.py",
            "--run-manifest",
            str(run_manifest_fp),
            "--output-report",
            str(leak_fp),
    ...
    _run(
        [
            ...
            "--output-slices",
            str(out_root / "h2_train_aware_region_slices.csv"),
            "--output-summary",
            str(out_root / "h2_train_aware_region_summary.csv"),
            "--output-balanced-slices",
            str(out_root / "h2_train_aware_balanced_region_slices.csv"),
            "--output-balanced-summary",
            str(out_root / "h2_train_aware_balanced_region_summary.csv"),
            "--balanced-n-total",
            str(int(args.balanced_n_total)),
            "--balanced-min-full-region",
            str(int(args.balanced_min_full_region)),
            ...
            str(out_root / "h2_train_aware_balanced_manifest.csv"),
            "--output-balanced-manifest-summary",
            str(out_root / "h2_train_aware_balanced_manifest_summary.csv"),
        ]
    )
    ...
    for n in [24, 36]:
        ...
        if (min_full + min_stress + min_mk3) > int(n_use):
            ...
            continue
        sweep_prefix = out_root / f"dev_lock_sweep_n{n}_h2trainaware"
        cmd = [
            py,
