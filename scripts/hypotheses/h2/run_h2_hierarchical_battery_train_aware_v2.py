"""Build H2 battery-long table with per-triplet retraining (train-aware check).

H2 idea (literature-aligned): hierarchical known-region routing.
1) Train base models with triplet holdout (center1500 + center2900).
2) Build per-sample probability features from both centers.
3) Train class-heads on CV only:
   - global head;
   - optional region-specific heads (cortex/striatum/cerebellum).
4) Apply routed prediction to true holdout rows only.
"""
...
from raman_hack.experimental.hierarchical import (  # noqa: E402
    fit_known_region_hierarchical_heads,
    normalize_region,
)
...
def _ensure_region_column(sample_meta: pl.DataFrame) -> pl.DataFrame:
    if "region" in sample_meta.columns:
        if "sample_id" in sample_meta.columns:
            return sample_meta.with_columns(
                ...
                .map_elements(
                    lambda x: normalize_region(str(x.get("region", "")))
                    if normalize_region(str(x.get("region", ""))) != "unknown"
                    else _region_from_sample_id(str(x.get("sample_id", ""))),
        ...
        return sample_meta.with_columns(
            pl.col("region")
            .map_elements(lambda v: normalize_region(str(v)), return_dtype=pl.Utf8)
            .alias("region")
        )
    if "sample_id" in sample_meta.columns:
        return sample_meta.with_columns(
    ...
    raise ValueError("sample_meta has no region and cannot infer it.")
...
def _load_predictions_with_split(run_dir: Path) -> pl.DataFrame:
    fp = run_dir / "predictions.parquet"
    if not fp.exists():
        raise FileNotFoundError(f"Missing predictions.parquet: {fp}")
    d = pl.read_parquet(fp)
    need = ["split", "sample_id", "mouse", "class_label", "y_true", *PROBA_COLS]
    ...
    out = (
        d.select(["split", "sample_id", "mouse", "class_label", "y_true", "region", *PROBA_COLS])
        .with_columns(
            _sample_key_expr("sample_id").alias("sample_key"),
            pl.struct(["region", "sample_id"])
            .map_elements(
                lambda x: normalize_region(str(x["region"]))
                if normalize_region(str(x["region"])) != "unknown"
                else _region_from_sample_id(str(x["sample_id"])),
                return_dtype=pl.Utf8,
            )
            .alias("region"),
        )
        .select(["split", "sample_key", "mouse", "class_label", "y_true", "region", *PROBA_COLS])
    )
    return out


def _ensemble_predictions(preds: list[pl.DataFrame], *, split: str, top_k: int) -> pl.DataFrame:
    if not preds:
        raise ValueError("preds must be non-empty")
    k = _normalize_topk(requested=top_k, n_models=len(preds))
    use = [p.filter(pl.col("split") == split) for p in preds[:k]]
    use = [p for p in use if p.height > 0]
    if not use:
        raise ValueError(f"No rows for split={split}")
    if len(use) == 1:
        return use[0].drop("split")
    all_df = pl.concat(use, how="vertical_relaxed")
    return (
        all_df.group_by(["sample_key", "mouse", "class_label", "y_true", "region"])
        .agg([pl.col(c).mean().alias(c) for c in PROBA_COLS])
        .select(["sample_key", "mouse", "class_label", "y_true", "region", *PROBA_COLS])
    )
...
def _build_feature_table(e1500: pl.DataFrame, e2900: pl.DataFrame) -> tuple[pl.DataFrame, list[str]]:
    f1500 = {c: f"p1500_{c}" for c in PROBA_COLS}
    f2900 = {c: f"p2900_{c}" for c in PROBA_COLS}
    j = e1500.rename(f1500).join(
        e2900.select(["sample_key"] + PROBA_COLS).rename(f2900),
        on="sample_key",
        how="inner",
    )
    feat_cols = [f"p1500_{c}" for c in PROBA_COLS] + [f"p2900_{c}" for c in PROBA_COLS]
    return j.select(["sample_key", "mouse", "class_label", "y_true", "region", *feat_cols]), feat_cols


def _predict_h2(df_feat: pl.DataFrame, feat_cols: list[str], proba: np.ndarray) -> pl.DataFrame:
    p = pl.DataFrame(
        {
            "sample_key": df_feat["sample_key"],
            "mouse": df_feat["mouse"],
            "class_label": df_feat["class_label"],
            "y_true": df_feat["y_true"],
            "region": df_feat["region"],
            "proba_control": proba[:, 0],
            "proba_endo": proba[:, 1],
            "proba_exo": proba[:, 2],
        }
    )
    return p.select(["sample_key", "mouse", "class_label", "y_true", "region", *PROBA_COLS])
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train-aware H2 check: retrain per holdout triplet and build battery-long."
    )
    ...
    parser.add_argument("--configs-1500", default="")
    parser.add_argument("--configs-2900", default="")
    parser.add_argument("--output-root", default="runs/analysis/hyp_v2_h2_train_aware_20260307")
    parser.add_argument("--holdout-manifest", default="")
    ...
    parser.add_argument("--h2-topk-1500", type=int, default=3)
    parser.add_argument("--h2-topk-2900", type=int, default=4)
    parser.add_argument("--h2-logreg-c", type=float, default=1.0)
    parser.add_argument("--h2-logreg-max-iter", type=int, default=2000)
    parser.add_argument("--h2-min-rows-per-region", type=int, default=12)
    parser.add_argument("--h2-min-classes-per-region", type=int, default=2)
    parser.add_argument("--h2-name", default="h2_known_region_hier_lr")
    parser.add_argument("--include-h2-global-control", action="store_true")
    ...
    parser.add_argument("--output-head-manifest", default="")
    ...
    train_runs_root = out_root / "train_runs"
    tmp_cfg_dir = out_root / "tmp_configs"
    out_battery = Path(args.output_battery) if args.output_battery else out_root / "h2_train_aware_battery.csv"
    out_summary = Path(args.output_summary) if args.output_summary else out_root / "h2_train_aware_summary.csv"
    out_run_manifest = (
        Path(args.output_run_manifest)
        if args.output_run_manifest
        else out_root / "h2_train_aware_run_manifest.csv"
    )
    out_head_manifest = (
        Path(args.output_head_manifest)
        if args.output_head_manifest
        else out_root / "h2_train_aware_head_manifest.csv"
    )
    out_cfg = Path(args.output_config) if args.output_config else out_root / "h2_train_aware_config.json"
    ...
    h2_k1500 = _normalize_topk(int(args.h2_topk_1500), len(cfgs_1500))
    h2_k2900 = _normalize_topk(int(args.h2_topk_2900), len(cfgs_2900))
    ...
    for i, t in enumerate(triplets.select(HOLDOUT_KEY).to_dicts(), start=1):
        ...
        for idx, cfg_path in enumerate(cfgs_1500, start=1):
            refs_1500.append(
                _run_one_model(
                    ...
                    tmp_cfg_dir=tmp_cfg_dir,
                    suffix=f"h2ta_{trip_key}_s{seed_i}",
                )
            )
        ...
        pred_1500: list[pl.DataFrame] = []
        pred_2900: list[pl.DataFrame] = []

        for r in refs_1500:
            d = _load_predictions_with_split(Path(r.run_dir))
            pred_1500.append(d)
            run_rows.append(
                {
                    "holdout_control": c,
                    ...
                    "run_id": r.run_id,
                    "run_dir": r.run_dir,
                    "n_cv_rows": int(d.filter(pl.col("split") == "cv").height),
                    "n_holdout_rows": int(d.filter(pl.col("split") == "holdout").height),
                }
            )
        for r in refs_2900:
            d = _load_predictions_with_split(Path(r.run_dir))
        ...
        fb1500 = _ensemble_predictions(pred_1500, split="holdout", top_k=fb_k1500)
        fb2900 = _ensemble_predictions(pred_2900, split="holdout", top_k=fb_k2900)
        pr1500 = _ensemble_predictions(pred_1500, split="holdout", top_k=pr_k1500)
        pr2900 = _ensemble_predictions(pred_2900, split="holdout", top_k=pr_k2900)
        ...
        h2_cv_1500 = _ensemble_predictions(pred_1500, split="cv", top_k=h2_k1500)
        h2_cv_2900 = _ensemble_predictions(pred_2900, split="cv", top_k=h2_k2900)
        h2_hold_1500 = _ensemble_predictions(pred_1500, split="holdout", top_k=h2_k1500)
        h2_hold_2900 = _ensemble_predictions(pred_2900, split="holdout", top_k=h2_k2900)
        h2_cv_feat, feat_cols = _build_feature_table(h2_cv_1500, h2_cv_2900)
        h2_hold_feat, _ = _build_feature_table(h2_hold_1500, h2_hold_2900)

        train_x = h2_cv_feat.select(feat_cols).to_numpy()
        train_y = h2_cv_feat["y_true"].to_numpy().astype(int)
        train_regions = [str(v) for v in h2_cv_feat["region"].to_list()]
        model, fit_meta = fit_known_region_hierarchical_heads(
            train_x=train_x,
            train_y=train_y,
            train_regions=train_regions,
            feature_cols=feat_cols,
            logreg_c=float(args.h2_logreg_c),
            logreg_max_iter=int(args.h2_logreg_max_iter),
            min_rows_per_region=int(args.h2_min_rows_per_region),
            min_classes_per_region=int(args.h2_min_classes_per_region),
            seed=int(seed_i),
        )

        hold_x = h2_hold_feat.select(feat_cols).to_numpy()
        hold_regions = [str(v) for v in h2_hold_feat["region"].to_list()]
        p_h2 = model.predict_proba(hold_x, hold_regions)
        h2_pred = _predict_h2(h2_hold_feat, feat_cols, p_h2)

        strategy_map: dict[str, pl.DataFrame] = {
            str(args.fallback_name): fallback,
            str(args.primary_name): primary,
            str(args.h2_name): h2_pred,
        }
        if bool(args.include_h2_global_control):
            p_global = model.global_head.predict_proba(hold_x, n_classes=len(CLASS_ORDER))
            strategy_map["h2_global_head_control"] = _predict_h2(h2_hold_feat, feat_cols, p_global)
        ...
        for region, info in fit_meta["regions"].items():
            head_rows.append(
                {
                    "holdout_control": c,
                    "holdout_endo": e,
                    ...
                    "triplet_seed": seed_i,
                    "region": str(region),
                    "n_rows_cv_region": int(info["n_rows"]),
                    "n_classes_cv_region": int(info["n_classes"]),
                    "head_source": str(info["source"]),
                }
            )

        print(
            f"[{i}/{triplets.height}] {trip_key}: "
            f"runs1500={len(refs_1500)}, runs2900={len(refs_2900)}, "
            f"cv_rows={h2_cv_feat.height}, hold_rows={h2_hold_feat.height}"
        )
    ...
    pl.DataFrame(head_rows).write_csv(out_head_manifest)
    ...
    cfg_payload = {
        ...
        "h2_topk_1500": int(h2_k1500),
        "h2_topk_2900": int(h2_k2900),
        "h2_logreg_c": float(args.h2_logreg_c),
        "h2_logreg_max_iter": int(args.h2_logreg_max_iter),
        "h2_min_rows_per_region": int(args.h2_min_rows_per_region),
        "h2_min_classes_per_region": int(args.h2_min_classes_per_region),
        "h2_name": str(args.h2_name),
        "include_h2_global_control": bool(args.include_h2_global_control),
    ...
    print(f"Saved head manifest: {out_head_manifest}")
