"""Audit train-aware H1 run-manifest for holdout/cv leakage issues."""
...
def _iter_manifest_runs(man: pl.DataFrame) -> list[dict[str, str]]:
    out: list[dict[str, str]] = []
    cols = set(man.columns)
    # New long format.
    if {"run_dir", "center"}.issubset(cols):
        for r in man.to_dicts():
            out.append(
                {
                    "center": str(r.get("center", "")),
                    "run_dir": str(r.get("run_dir", "")),
                    "holdout_control": str(r.get("holdout_control", "")),
                    "holdout_endo": str(r.get("holdout_endo", "")),
                    "holdout_exo": str(r.get("holdout_exo", "")),
                }
            )
        return out

    # Backward-compatible wide format.
    need = {"run_dir_1500", "run_dir_2900", "holdout_control", "holdout_endo", "holdout_exo"}
    if not need.issubset(cols):
        raise ValueError(
            "Unknown run-manifest format. Expected either "
            "{run_dir,center,...} or {run_dir_1500,run_dir_2900,...}."
        )
    for r in man.to_dicts():
        hc = str(r["holdout_control"])
        he = str(r["holdout_endo"])
        hx = str(r["holdout_exo"])
        out.append(
            {
                "center": "1500",
                "run_dir": str(r["run_dir_1500"]),
                "holdout_control": hc,
                "holdout_endo": he,
                "holdout_exo": hx,
            }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Leakage audit for H1 train-aware runs.")
    parser.add_argument("--run-manifest", required=True)
    parser.add_argument("--output-report", required=True)
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Exit with code 2 if any run fails audit.",
    )
    ...
    for item in _iter_manifest_runs(man):
