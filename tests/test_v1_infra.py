...
def test_compare_and_sweep_smoke(tmp_path: Path) -> None:
    ...
    cfg_template = {
        ...
        "model": {"iterations": 8, "learning_rate": 0.2, "depth": 3, "verbose": False},
        "validation": {"n_splits": 3},
    }
    ...
    cfg2["model"] = dict(cfg_template["model"], iterations=6)
    ...
    subprocess.check_call(sweep_cmd, cwd=str(ROOT))
    ...
    subprocess.check_call(compare_cmd, cwd=str(ROOT))
