...
def norm_vector(X: np.ndarray) -> np.ndarray:
    l2 = np.sqrt((X * X).sum(axis=1, keepdims=True))
    return X / (l2 + 1e-12)


def norm_auc(X: np.ndarray) -> np.ndarray:
    # Absolute area is more stable after baseline/derivative steps.
    trapz_fn = getattr(np, "trapezoid", None)
    if trapz_fn is None:
        trapz_fn = np.trapz
    area = trapz_fn(np.abs(X), axis=1).reshape(-1, 1)
    return X / (area + 1e-12)


def norm_max(X: np.ndarray) -> np.ndarray:
