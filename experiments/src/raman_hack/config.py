...
class PreprocessConfig:
    wn_min: float = 600.0
    wn_max: float = 1800.0
    baseline_method: str = "arpls"  # none|als|asls|airpls|arpls|snip|morph
    ...
    normalization: str = "snv"  # none|minmax|snv|msc|emsc
    ...
    def from_dict(cls, d: dict[str, Any]) -> "PreprocessConfig":
        ...
        _require(
            obj.baseline_method in {"none", "als", "asls", "airpls", "arpls", "snip", "morph"},
            "preprocess.baseline_method must be one of none|als|asls|airpls|arpls|snip|morph",
        )
        _require(
            obj.normalization in {"none", "minmax", "snv", "msc", "emsc"},
            "preprocess.normalization must be one of none|minmax|snv|msc|emsc",
        )
        _require(
...
@dataclass
class ModelConfig:
    model_family: str = "catboost"  # catboost|logreg|svm_rbf|resnet1d|ramannet|spectral_transformer|raman_moe|inception1d|drsn1d|efficientnet1d|single_step_residual|single_step_unet|rgt_pipeline
    ...
    verbose: bool = False
    # Torch training controls (for resnet1d|ramannet|spectral_transformer|raman_moe|rgt_pipeline)
    torch_epochs: int = 40
    ...
    transformer_frontend: str = "raw"  # raw|multitaper
    multitaper_nw: float = 3.0
    multitaper_n_tapers: int = 5
    multitaper_remove_dc: bool = True
    # Raman MoE hyper-params
    moe_emb_channels: int = 64
    moe_n_experts: int = 3
    moe_expert_hidden: int = 64
    moe_use_sectors_for_gating: bool = True
    moe_sector_reduce: str = "mean"  # mean|sum|max
    moe_learnable_proj: bool = True
    moe_proj_dim: int = 32
    moe_n_freq_bins: int = 0  # <=0 means use n_features
    moe_sector_source: str = "hybrid"  # hybrid|yaml|index
    moe_sector_bounds: list[Any] = field(default_factory=list)
    ...
    # RGT pipeline hyper-params
    rgt_backbone: str = "auto"  # auto|ramannet|spectral_transformer
    rgt_unmix_method: str = "nfindr_nnls"  # nfindr_nnls|nmf_nnls
    rgt_unmix_k: int = 5
    rgt_pretrain_gan_epochs: int = 8
    rgt_pretrain_denoiser_epochs: int = 12
    rgt_joint_epochs: int = 15
    rgt_lambda_cls: float = 1.0
    rgt_lambda_recon: float = 0.2
    rgt_lambda_bg: float = 0.1
    rgt_lambda_abund: float = 0.05
    rgt_teacher_baseline_method: str = "arpls"  # none|als|asls|airpls|arpls|snip|morph
    rgt_use_l2_norm: bool = True
    ...
    def from_dict(cls, d: dict[str, Any]) -> "ModelConfig":
        ...
        rgt_unmix_method_raw = str(d.get("rgt_unmix_method", "nfindr_nnls")).lower()
        if rgt_unmix_method_raw == "nfindr":
            rgt_unmix_method_raw = "nfindr_nnls"
        elif rgt_unmix_method_raw == "nmf":
            rgt_unmix_method_raw = "nmf_nnls"
        obj = cls(
            ...
            transformer_frontend=str(d.get("transformer_frontend", "raw")),
            multitaper_nw=float(d.get("multitaper_nw", 3.0)),
            multitaper_n_tapers=int(d.get("multitaper_n_tapers", 5)),
            multitaper_remove_dc=bool(d.get("multitaper_remove_dc", True)),
            moe_emb_channels=int(d.get("moe_emb_channels", 64)),
            moe_n_experts=int(d.get("moe_n_experts", 3)),
            moe_expert_hidden=int(d.get("moe_expert_hidden", 64)),
            moe_use_sectors_for_gating=bool(d.get("moe_use_sectors_for_gating", True)),
            moe_sector_reduce=str(d.get("moe_sector_reduce", "mean")),
            moe_learnable_proj=bool(d.get("moe_learnable_proj", True)),
            moe_proj_dim=int(d.get("moe_proj_dim", 32)),
            moe_n_freq_bins=int(d.get("moe_n_freq_bins", 0)),
            moe_sector_source=str(d.get("moe_sector_source", "hybrid")),
            moe_sector_bounds=list(d.get("moe_sector_bounds", [])),
            ...
            rgt_backbone=str(d.get("rgt_backbone", "auto")),
            rgt_unmix_method=rgt_unmix_method_raw,
            rgt_unmix_k=int(d.get("rgt_unmix_k", 5)),
            rgt_pretrain_gan_epochs=int(d.get("rgt_pretrain_gan_epochs", 8)),
            rgt_pretrain_denoiser_epochs=int(d.get("rgt_pretrain_denoiser_epochs", 12)),
            rgt_joint_epochs=int(d.get("rgt_joint_epochs", 15)),
            rgt_lambda_cls=float(d.get("rgt_lambda_cls", 1.0)),
            rgt_lambda_recon=float(d.get("rgt_lambda_recon", 0.2)),
            rgt_lambda_bg=float(d.get("rgt_lambda_bg", 0.1)),
            rgt_lambda_abund=float(d.get("rgt_lambda_abund", 0.05)),
            rgt_teacher_baseline_method=str(
                d.get("rgt_teacher_baseline_method", "arpls")
            ).lower(),
            rgt_use_l2_norm=bool(d.get("rgt_use_l2_norm", True)),
        )
        _require(
            obj.model_family
            in {
            ...
            "model.model_family must be one of catboost|logreg|svm_rbf|resnet1d|ramannet|spectral_transformer|raman_moe|inception1d|drsn1d|efficientnet1d|single_step_residual|single_step_unet|rgt_pipeline",
        )
        ...
        _require(
            obj.torch_device in {"auto", "cpu", "cuda"},
            "model.torch_device must be one of auto|cpu|cuda",
        )
        ...
        _require(
            obj.transformer_frontend in {"raw", "multitaper"},
            "model.transformer_frontend must be raw|multitaper",
        )
        _require(obj.multitaper_nw > 0, "model.multitaper_nw must be > 0")
        _require(
            obj.multitaper_n_tapers >= 1,
            "model.multitaper_n_tapers must be >= 1",
        )
        _require(obj.moe_emb_channels >= 4, "model.moe_emb_channels must be >= 4")
        _require(obj.moe_n_experts >= 1, "model.moe_n_experts must be >= 1")
        _require(obj.moe_expert_hidden >= 4, "model.moe_expert_hidden must be >= 4")
        _require(
            obj.moe_sector_reduce in {"mean", "sum", "max"},
            "model.moe_sector_reduce must be mean|sum|max",
        )
        _require(obj.moe_proj_dim >= 4, "model.moe_proj_dim must be >= 4")
        _require(
            obj.moe_sector_source in {"hybrid", "yaml", "index"},
            "model.moe_sector_source must be hybrid|yaml|index",
        )
        _require(
            isinstance(obj.moe_sector_bounds, list),
            "model.moe_sector_bounds must be a list",
        )
        _require(
        ...
        _require(
            obj.rgt_backbone in {"auto", "ramannet", "spectral_transformer"},
            "model.rgt_backbone must be auto|ramannet|spectral_transformer",
        )
        _require(
            obj.rgt_unmix_method in {"nfindr_nnls", "nmf_nnls"},
            "model.rgt_unmix_method must be nfindr_nnls|nmf_nnls",
        )
        _require(obj.rgt_unmix_k >= 2, "model.rgt_unmix_k must be >= 2")
        _require(
            obj.rgt_pretrain_gan_epochs >= 1,
            "model.rgt_pretrain_gan_epochs must be >= 1",
        )
        _require(
            obj.rgt_pretrain_denoiser_epochs >= 1,
            "model.rgt_pretrain_denoiser_epochs must be >= 1",
        )
        _require(obj.rgt_joint_epochs >= 1, "model.rgt_joint_epochs must be >= 1")
        _require(obj.rgt_lambda_cls > 0, "model.rgt_lambda_cls must be > 0")
        _require(obj.rgt_lambda_recon >= 0, "model.rgt_lambda_recon must be >= 0")
        _require(obj.rgt_lambda_bg >= 0, "model.rgt_lambda_bg must be >= 0")
        _require(obj.rgt_lambda_abund >= 0, "model.rgt_lambda_abund must be >= 0")
        _require(
            obj.rgt_teacher_baseline_method
            in {"none", "als", "asls", "airpls", "arpls", "snip", "morph"},
            "model.rgt_teacher_baseline_method must be one of none|als|asls|airpls|arpls|snip|morph",
        )
        return obj
...
class ExplainabilityConfig:
    ...
    target_models: list[str] = field(default_factory=lambda: ["folds", "final"])
    target_split: str = "cv"
    sample_limit: int = 16
    report_top_bands: int = 5
    ig_steps: int = 32
    lrp_rule: str = "epsilon"
    artifact_threshold: float = 0.05
    band_registry: str = "auto"
    save_raw: bool = True
    ...
    def from_dict(cls, d: dict[str, Any]) -> "ExplainabilityConfig":
        obj = cls(
            ...
            methods=[str(v) for v in d.get("methods", ["integrated_gradients"])],
            target_models=[str(v) for v in d.get("target_models", ["folds", "final"])],
            target_split=str(d.get("target_split", "cv")),
            sample_limit=int(d.get("sample_limit", 16)),
            report_top_bands=int(d.get("report_top_bands", 5)),
            ig_steps=int(d.get("ig_steps", 32)),
            lrp_rule=str(d.get("lrp_rule", "epsilon")),
            artifact_threshold=float(d.get("artifact_threshold", 0.05)),
            band_registry=str(d.get("band_registry", "auto")),
            save_raw=bool(d.get("save_raw", True)),
            save_validation=bool(d.get("save_validation", True)),
        )
        _require(obj.sample_limit >= 1, "explainability.sample_limit must be >= 1")
        _require(obj.report_top_bands >= 1, "explainability.report_top_bands must be >= 1")
        _require(obj.ig_steps >= 2, "explainability.ig_steps must be >= 2")
        _require(
            obj.target_split in {"cv", "all"},
            "explainability.target_split must be one of cv|all",
        )
        _require(
            0.0 <= obj.artifact_threshold <= 1.0,
            "explainability.artifact_threshold must be in [0, 1]",
        )
        return obj
...
class AppConfig:
    ...
    def from_dict(cls, d: dict[str, Any]) -> "AppConfig":
        return cls(
            ...
            explainability=ExplainabilityConfig.from_dict(d.get("explainability", {})),
        )
