...
def test_explain_batch_cli_smoke(tmp_path: Path) -> None:
    data_root = _make_small_real_tree(tmp_path)
    cfg = {
        "experiment": {
            "name": "explain_batch_smoke",
            "seed": 17,
    ...
    assert any((run_dir / "interpretability" / "raw" / "integrated_gradients").rglob("sample_*.json"))
    assert any((run_dir / "interpretability" / "raw" / "gradcam1d").rglob("sample_*.png"))


def test_explain_batch_cli_with_raman_moe_sector_artifacts(tmp_path: Path) -> None:
    data_root = _make_small_real_tree(tmp_path)
    cfg = {
        "experiment": {
            "name": "explain_batch_moe_smoke",
            "seed": 21,
            "output_root": str(tmp_path / "runs"),
        },
        "data": {
    ...
    json_files = list((run_dir / "interpretability" / "raw" / "integrated_gradients").rglob("sample_*.json"))
    assert json_files
    payload = json.loads(json_files[0].read_text(encoding="utf-8"))
    assert isinstance(payload.get("sector_attribution"), list)
    assert isinstance(payload.get("sector_ranges_cm1"), list)


def test_explain_batch_cli_with_rgt_abundance_artifacts(tmp_path: Path) -> None:
    data_root = _make_small_real_tree(tmp_path)
    cfg = {
        "experiment": {
            "name": "explain_batch_rgt_smoke",
            "seed": 25,
            "output_root": str(tmp_path / "runs"),
        },
        "data": {
    ...
    json_files = list(
        (run_dir / "interpretability" / "raw" / "integrated_gradients").rglob("sample_*.json")
    )
    assert json_files
    payload = json.loads(json_files[0].read_text(encoding="utf-8"))
    assert isinstance(payload.get("abundance_vector"), list)
    assert isinstance(payload.get("top_abundance_components"), list)
